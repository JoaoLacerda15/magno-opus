...
// Removida a importação de useRoute, pois não precisamos mais dos params
import { Ionicons, MaterialCommunityIcons } from "@expo/vector-icons";
// import AuthService from "../../services/authService"; // Não é mais necessário aqui
import BarraNavegacao from "../../components/navbar";
import { useUser } from "../../hooks/useUser"; // ✅ Importação do hook

// Removida a instância de AuthService: const auth = new AuthService();

export default function PerfilPP() {
  // Removida a declaração de 'route'
  const navigation = useNavigation();
  // Removida a declaração de 'userId'

  // 1. ✅ USO DO HOOK SEM PARÂMETROS
  const { user, loading, error } = useUser();

  // 2. ✅ TRATAMENTO DE ESTADO DE CARREGAMENTO
  if (loading) {
    return (
...
  // 3. ✅ TRATAMENTO DE ESTADO DE ERRO (Inclui usuário não logado)
  if (error || !user) {
    return (
      <View style={styles.statusContainer}>
        <Text style={styles.statusText}>{error || "Erro desconhecido ao carregar o perfil."}</Text>
        {/* Você pode adicionar um botão de login aqui, se desejar */}
      </View>
    );
  }
...
  scrollView: { flex: 1 },
  ...
  // ✅ ESTILOS ADICIONAIS PARA MENSAGENS DE STATUS
  statusContainer: {
    flex: 1,
